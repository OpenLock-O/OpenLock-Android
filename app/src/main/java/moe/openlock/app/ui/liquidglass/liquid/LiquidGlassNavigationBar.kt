...
import moe.openlock.app.ui.liquidglass.animation.DampedDragAnimation
import moe.openlock.app.ui.liquidglass.animation.InteractiveHighlight
...
/**
 * Liquid-glass pill bar, ported from the miuix sample app.
 *
 * Icon plus an 11sp label per tab, a refraction lens over the blurred
 * backdrop, a gravity-rotated edge highlight, and a press indicator that
 * follows the finger with a damped drag. The filled `containerColor` is only
 * drawn when the blur is off - with the blur on the frost *is* the surface.
 */
@Composable
fun LiquidGlassNavigationBar(
    ...
    // The sample reads its own theme switch; this app derives it from the
    // palette so the bar matches whatever theme (including AMOLED) is active.
